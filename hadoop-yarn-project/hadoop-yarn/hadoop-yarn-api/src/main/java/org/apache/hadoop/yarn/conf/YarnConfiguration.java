...
public class YarnConfiguration extends Configuration {
  ...
  /**
   * If the port should be included or not in the node name. The node name is
   * used by the scheduler for resource requests allocation location matching.
   * Typically this is just the hostname, using the port is needed when using
   * minicluster and specific NM are required.
   */
   ...
   */
  public static final String RM_GROUP_MEMBERSHIP_ADDRESS =
      RM_PREFIX + "groupMembership.address";
  public static final String RM_GROUP_MEMBERSHIP_PORT =
   ...
   */
  public static final String RM_GROUP_MEMBERSHIP_CLIENT_THREAD_COUNT =
      RM_PREFIX + "admin.client.thread-count";
  ...
  public static final int
      DEFAULT_RM_HISTORY_WRITER_MULTI_THREADED_DISPATCHER_POOL_SIZE = 10;
  ...
  public static final String FS_BASED_RM_CONF_STORE =
      RM_PREFIX + "configuration.file-system-based-store";
  ...
  public static final String RM_CONFIGURATION_PROVIDER_CLASS =
      RM_PREFIX + "configuration.provider-class";
  ...
  public static final String DISTRIBUTED_CLIENT_FAILOVER_PROXY_PROVIDER =
      CLIENT_FAILOVER_PREFIX + "proxy-provider";
  public static final String
      DEFAULT_DISTRIBUTED_CLIENT_FAILOVER_PROXY_PROVIDER =
  ...
  public static final String FS_RM_STATE_STORE_RETRY_POLICY_SPEC =
      RM_PREFIX + "fs.state-store.retry-policy-spec";
  ...
  public static final long
      DEFAULT_RM_AMRM_TOKEN_MASTER_KEY_ROLLING_INTERVAL_SECS = 24 * 60 * 60;
  public static final String
  ...
  /**
   * Adjustment to make to the container os scheduling priority. The valid
   * values for this could vary depending on the platform. On Linux, higher
   * values mean run the containers at a less favorable priority than the NM.
  ...
  public static final long
      DEFAULT_RM_DELAYED_DELEGATION_TOKEN_REMOVAL_INTERVAL_MS = 30000l;
  /**
  ...
  public static final String LOG_AGGREGATION_ENABLED =
      YARN_PREFIX + "log-aggregation-enable";
  public static final boolean DEFAULT_LOG_AGGREGATION_ENABLED = false;
  /**
   * How long to wait before deleting aggregated logs, -1 disables. Be careful
  ...
  /**
   * How long to wait between aggregated log retention checks. If set to a
   * value <= 0 then the value is computed as one-tenth of the log retention
  ...
  public static final String PROCFS_USE_SMAPS_BASED_RSS_ENABLED =
      NM_PREFIX + ".container-monitor.procfs-tree.smaps-based-rss.enabled";
  ...
  /**
   * The maximum percentage of disk space that can be used after which a disk
   * is marked as offline. Values can range from 0.0 to 100.0. If the value is
   * greater than or equal to 100, NM will check for full disk. This applies
   * to nm-local-dirs and nm-log-dirs.
   */
  ...
  public static final float DEFAULT_NM_MAX_PER_DISK_UTILIZATION_PERCENTAGE =
      100.0F;
  /**
   * The minimum space that must be available on a local dir for it to be
   * used. This applies to nm-local-dirs and nm-log-dirs.
   */
  ...
  /**
   * The UNIX group that the linux-container-executor should run as. This is
   * intended to be set as part of container-executor.cfg.
   */
  ...
  public static final String NM_NONSECURE_MODE_LOCAL_USER_KEY =
      NM_PREFIX + "linux-container-executor.nonsecure-mode.local-user";
  ...
  public static final String NM_NONSECURE_MODE_USER_PATTERN_KEY =
      NM_PREFIX + "linux-container-executor.nonsecure-mode.user-pattern";
  ...
  /**
   * No. of milliseconds to wait between sending a SIGTERM and SIGKILL to a
   * running container
   */
  ...
  public static final String YARN_APPLICATION_CLASSPATH =
      YARN_PREFIX + "application.classpath";
  /**
   ...
   * will be replaced with real parameter expansion marker ('%' for Windows
   * and '$' for Linux) by NodeManager on container launch. For example:
   * {{VAR}} will be replaced as $VAR on Linux, and %VAR% on Windows.
   */
  @Public
  @Unstable
  public static final String[]
   ...
   * to a Linux/Unix server or vice versa.
   * </p>
   */
  ...
  /**
   * Whether the NM should use RPC to connect to the RM. Default is false. Can
   * be set to true only when using fixed ports.
   */
  public static final String YARN_MINICLUSTER_USE_RPC =
      YARN_MC_PREFIX + "use-rpc";
  ...
  /**
   * The setting that controls whether application history service is enabled
   * or not.
   */
  @Private
  ...
  ////////////////////////////////
  /**
   * Use YARN_CLIENT_APPLICATION_CLIENT_PROTOCOL_POLL_INTERVAL_MS instead. The
   * interval of the yarn client's querying application state after
  ...
  public static final long
      DEFAULT_YARN_CLIENT_APPLICATION_CLIENT_PROTOCOL_POLL_INTERVAL_MS = 200;
  /**
   * The duration that the yarn client library waits, cumulatively across
   * polls, for an expected state change to occur. Defaults to -1, which
   * indicates no limit.
   */
  public static final String
  ...
  public static final long
      DEFAULT_YARN_CLIENT_APPLICATION_CLIENT_PROTOCOL_POLL_TIMEOUT_MS = -1;
  /**
   ...
   * Ex. configuring it to a value of 5 will make sure that client will at max
   * have 5 connections cached with 5 different node managers. These
   ...
   * connection should be closed in order to use newer token. Note:
   * {@link YarnConfiguration#NM_CLIENT_ASYNC_THREAD_POOL_MAX_SIZE} are
 ...
 //TODO What is this for? does it have to be cleaned
  ...
  public static final List<String> DEFAULT_RM_ADMIN_ADDRESS_HOP;
  public static final String TESTING_PARAMETERS_PATH =
      "./src/test/resources/testing-params.properties";
  public static final Map<Integer, Integer> RESOURCE_TRACKER_SERVICE_PORTS =
      new HashMap<Integer, Integer>();
  public static final List<String> RM_RESOURCE_TRACKER_ADDRESS_HOP;
  //Used By RMContextImpl to init the Client with the Scheduler ResourceTrackerService
  public static String DEFAULT_RM_RESOURCE_TRACKER_ADDRESS_HOP_CLOUD = "";
  public static final int NUMBER_OF_RM_SECONDARY = 50;
  public static final int NUMBER_OF_RM = NUMBER_OF_RM_SECONDARY + 1;
  public static final List<Integer> DEFAULT_RM_RESOURCE_TRACKER_PORT_HOP =
  ...
  static {
    ...
    RESOURCE_TRACKER_SERVICE_PORTS.put(0, 15000);

    RM_RESOURCE_TRACKER_ADDRESS_HOP = new ArrayList<String>();
    DEFAULT_RM_RESOURCE_TRACKER_ADDRESS_HOP = new ArrayList<String>();
    DEFAULT_RM_PORT_HOP = new ArrayList<Integer>();
    DEFAULT_RM_ADDRESS_HOP = new ArrayList<String>();
    DEFAULT_RM_ADMIN_PORT_HOP = new ArrayList<Integer>();
    DEFAULT_RM_ADMIN_ADDRESS_HOP = new ArrayList<String>();

    RM_RESOURCE_TRACKER_ADDRESS_HOP.add(RM_PREFIX + "resource-tracker.address");
    DEFAULT_RM_RESOURCE_TRACKER_PORT_HOP.add(DEFAULT_RM_RESOURCE_TRACKER_PORT);
    DEFAULT_RM_PORT_HOP.add(DEFAULT_RM_PORT);
    DEFAULT_RM_ADMIN_PORT_HOP.add(DEFAULT_RM_ADMIN_PORT);


    for (int i = 1; i < NUMBER_OF_RM; i++) {
      DEFAULT_RM_RESOURCE_TRACKER_PORT_HOP.add(16000 + i);
      DEFAULT_RM_PORT_HOP.add(12000 + i);
      DEFAULT_RM_ADMIN_PORT_HOP.add(13000 + i);
    }
    for (int i = 1; i < NUMBER_OF_RM; i++) {
   ...
   * <code>name</code> property as a
   * <code>InetSocketAddress</code>. On a HA cluster, this fetches the address
   * corresponding to the RM identified by {@link #RM_HA_ID}.
   *
   * @param name
   *     property name.
  ...
  @Override
  public int getPort(String name, int defaultPort) {
    if (HAUtil.isHAEnabled(this) &&
        getServiceAddressConfKeys(this).contains(name)) {
      return HAUtil.getConfValueForRMInstance(name, defaultPort, this);
    } else {

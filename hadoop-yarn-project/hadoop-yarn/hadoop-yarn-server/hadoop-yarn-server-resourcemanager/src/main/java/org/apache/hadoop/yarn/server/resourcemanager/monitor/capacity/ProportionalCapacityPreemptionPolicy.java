 ...
 * is needed. Overcapacity is distributed among queues in a weighted fair
 * manner,
...
public class ProportionalCapacityPreemptionPolicy
   ...
   *
   * @param root
   *     the root of the CapacityScheduler queue hierarchy
   * @param clusterResources
   *     the total amount of resources in the cluster
   */
  private void containerBasedPreemptOrKill(CSQueue root,
      Resource clusterResources, TransactionState transactionState) {
    ...
    Resource totalPreemptionAllowed = Resources
        .multiply(clusterResources, percentageClusterPreemptionAllowed);
    ...
    for (Map.Entry<ApplicationAttemptId, Set<RMContainer>> e : toPreempt
        .entrySet()) {
   ...
   *
   * @param root
   *     the root of the cloned queue hierachy
   ...
   */
  private List<TempQueue> recursivelyComputeIdealAssignment(TempQueue root,
   ...
   *     an out param)
   * @param totalPreemptionAllowed
   *     total amount of preemption we allow
   * @param tot_guarant
   *     the amount of capacity assigned to this pool of queues
   */
  private void computeIdealResourceDistribution(ResourceCalculator rc,
      List<TempQueue> queues, Resource totalPreemptionAllowed,
    ...
    if (Resources.greaterThan(rc, tot_guarant, totPreemptionNeeded,
        totalPreemptionAllowed)) {
      scalingFactor = Resources
          .divide(rc, tot_guarant, totalPreemptionAllowed, totPreemptionNeeded);
    }
   ...
   */
  private void computeFixpointAllocation(ResourceCalculator rc, Resource tot_guarant,
                                         Collection<TempQueue> qAlloc, Resource unassigned,
                                         boolean ignoreGuarantee) {
    // assign all cluster resources until no more demand, or no resources left
    while (!qAlloc.isEmpty() && Resources.greaterThan(rc, tot_guarant, unassigned,
            Resources.none())) {
   ...
   *
   * @param queues
   *     set of leaf queues to preempt from
   * @param clusterResource
   *     total amount of cluster resources
    ...
    for (RMContainer c : reservations) {
      ...
      if (!observeOnly) {
        dispatcher.handle(new ContainerPreemptEvent(appId, c,
            ContainerPreemptEventType.DROP_RESERVATION, transactionState));
      }
   ...
   * the leaves. Finally it aggregates pending resources in each queue and
   * rolls
  ...
  static class TempQueue {
    ...
    public String toString() {
      ...
      sb.append("NAME: " + queueName)
          ...
          .append(" GAR: ").append(guaranteed).append(" NORM: ")
          .append(normalizedGuarantee).append(" IDEAL_ASSIGNED: ")
          .append(idealAssigned).append(" IDEAL_PREEMPT: ")
          .append(toBePreempted).append(" ACTUAL_PREEMPT: ")
          .append(actuallyPreempted)
          .append("\n");
    ...
    public void assignPreemption(float scalingFactor, ResourceCalculator rc,
      ...
      if (Resources.greaterThan(rc, clusterResource, current, idealAssigned)) {
        toBePreempted = Resources
            .multiply(Resources.subtract(current, idealAssigned),
                scalingFactor);
      } else {
    ...
    void appendLogString(StringBuilder sb) {
      sb.append(queueName).append(", ").append(current.getMemory()).append(", ")
